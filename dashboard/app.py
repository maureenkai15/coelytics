...
with st.sidebar:
    st.markdown("""
    <div style='padding:1.5rem 0 1rem; border-bottom:1px solid #1e1e30; margin-bottom:1rem;'>
        <div style='font-size:1.4rem; font-weight:700; color:#f1f5f9; letter-spacing:-0.5px;'>COElytics</div>
        <div style='font-size:0.72rem; color:#334155; margin-top:2px; letter-spacing:0.05em; text-transform:uppercase;'>Singapore · Vehicle Intelligence</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style='border-top:1px solid #1e1e30; margin-top:1.5rem; padding-top:1rem;'>
        ...
        <div style='font-size:0.78rem; color:#475569; margin-top:2px;'>LTA · data.gov.sg</div>
    </div>
    """, unsafe_allow_html=True)
...
if page == "Market Overview":
    ...
    st.markdown('<div class="page-sub">Live COE premiums · Singapore Land Transport Authority</div>', unsafe_allow_html=True)
    ...
    with c1:
        ...
        if latest:
            ...
            fig2 = go.Figure(go.Bar(
                ...
                marker_line_width=0,
                text=df_d["ratio"].apply(lambda x: f"{x:.1f}×"),
    ...
    if not df.empty and sel:
        ...
        for cat in sel:
            ...
            fig.add_trace(go.Scatter(x=d["month"], y=d["premium"], name=cat,
                ...
                hovertemplate=f"<b>{cat}</b> · %{{x|%b %Y}}<br>$%{{y:,.0f}}<extra></extra>"))
...
# ══════════════════════════════════════════════════════════════
    ...
    c  = CATS[sel]
    st.markdown(f'<div class="page-sub" style="color:{c["color"]}88;">{c["desc"]}</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════
# PAGE 4 — Affordability Calculator
# ══════════════════════════════════════════════════════════════
    ...
    with c1:
        ...
        seg = st.selectbox("Segment", ["Mass market — Cat A (Toyota, Honda, Mazda)",
...
# ══════════════════════════════════════════════════════════════
    ...
    with c1:
        ...
        petrol    = st.number_input("Monthly petrol / charging (SGD)", value=200, step=10)
    ...
    with c2:
        ...
        <div class="kpi-wrap" style="margin-bottom:1rem;">
            ...
            <div class="kpi-meta">≈ <b style="color:#94a3b8">${mth:,.0f}</b> per month all-in</div>
        </div>
        """, unsafe_allow_html=True)
